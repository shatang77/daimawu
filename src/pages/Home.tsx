  ...
  return (
    <div className="p-5 sm:p-8 max-w-5xl mx-auto animate-in fade-in duration-500 overflow-x-hidden">
      ...
      <header className="mb-8 pt-4 relative">
        ...
        <h1 className="text-[2.5rem] font-black tracking-tight text-slate-800 mb-2 drop-shadow-sm">哈喽同学！👋</h1>
        <p className="text-slate-500 font-black text-xl sm:text-2xl mt-1 leading-snug">想找什么设计或模板？都在这里啦 💖</p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
        <div 
          ...
          className="bg-gradient-to-br from-[#FFB4D1] to-[#FF8FAB] rounded-[2rem] p-7 text-white shadow-[0_10px_30px_rgb(255,143,171,0.3)] cursor-pointer active:scale-95 transition-transform relative overflow-hidden"
        >
          <div className="absolute -right-4 -bottom-4 text-7xl opacity-30 rotate-[-15deg]">💻</div>
          <div className="text-5xl mb-4 relative z-10">✨</div>
          <h3 className="text-2xl font-black mb-1 relative z-10 drop-shadow-md">静态页面</h3>
          <p className="text-pink-100 font-bold items-center flex gap-1 relative z-10">HTML, CSS, JS <ArrowRight size={14}/></p>
        </div>
        
        <div 
          onClick={() => navigate('/list/ps')}
          className="bg-gradient-to-br from-[#A7C5EB] to-[#80A4F0] rounded-[2rem] p-7 text-white shadow-[0_10px_30px_rgb(128,164,240,0.3)] cursor-pointer active:scale-95 transition-transform relative overflow-hidden"
        >
          <div className="absolute -right-4 -bottom-4 text-7xl opacity-30 rotate-[-15deg]">🎨</div>
          <div className="text-5xl mb-4 relative z-10">🪄</div>
          <h3 className="text-2xl font-black mb-1 relative z-10 drop-shadow-md">PS 设计</h3>
          <p className="text-blue-100 font-bold items-center flex gap-1 relative z-10">高颜视觉稿 <ArrowRight size={14}/></p>
        </div>

        <div 
          onClick={() => navigate('/list/figma')}
          className="bg-gradient-to-br from-[#FCD49D] to-[#F1A25F] rounded-[2rem] p-7 text-white shadow-[0_10px_30px_rgb(241,162,95,0.3)] cursor-pointer active:scale-95 transition-transform relative overflow-hidden sm:col-span-2 lg:col-span-1"
        >
          <div className="absolute -right-4 -bottom-4 text-7xl opacity-30 rotate-[-15deg]">📐</div>
          <div className="text-5xl mb-4 relative z-10">🧸</div>
          <h3 className="text-2xl font-black mb-1 relative z-10 drop-shadow-md">Figma 设计</h3>
          <p className="text-orange-100 font-bold items-center flex gap-1 relative z-10">现代UI/体验 <ArrowRight size={14}/></p>
        </div>
      </div>
            ...
            {hotProducts.map(product => {
              ...
              return (
                <div 
                  ...
                  className="bg-white rounded-[1.5rem] overflow-hidden shadow-sm border border-pink-50/50 cursor-pointer active:scale-95 transition-all hover:shadow-xl hover:-translate-y-1 flex flex-col group p-2 min-w-[150px] w-[150px] sm:min-w-[180px] sm:w-[180px] shrink-0 snap-start"
                >
                  <div className="aspect-[4/3] bg-slate-50 rounded-[1.2rem] overflow-hidden relative">
                    ...
                    <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm text-pink-500 font-bold px-2 py-1 rounded-full text-xs shadow-sm">
                      库存 {inventory}
                    </div>
                  </div>
                  <div className="pt-3 pb-1 px-2">
                    <h3 className="text-sm font-extrabold text-slate-800 truncate mb-1" title={product.title}>{product.title}</h3>
                    <div className="flex justify-between items-end">
                      <span className="text-xs font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded-md">{product.theme}</span>
                    </div>
                  </div>
                </div>
              );
